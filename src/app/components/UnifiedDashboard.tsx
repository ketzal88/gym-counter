...
export default function UnifiedDashboard() {
  ...
  const month1Name = new Date(selectedYear, displayMonth1, 1).toLocaleDateString(dateLocale, { month: 'long', year: 'numeric' });
  const month2Name = new Date(selectedYear, displayMonth2, 1).toLocaleDateString(dateLocale, { month: 'long', year: 'numeric' });
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-white dark:bg-slate-950">
        <div className="flex flex-col items-center">
          ...
          <div className="text-sm text-slate-500 dark:text-slate-500">{t('common.loading')}</div>
        </div>
      </div>
    );
  }
    ...
    <div className="pb-24 max-w-lg mx-auto min-h-screen bg-white dark:bg-slate-950">
          ...
          <main className="px-6 py-8 space-y-8 animate-fade-in">
            <div className="space-y-2">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
                {t('dashboard.greeting').replace('{name}', user?.displayName?.split(' ')[0] || t('dashboard.greetingDefault'))} 👋
              </h2>
            ...
            <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 p-5">
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-500 uppercase tracking-wide">{t('dashboard.motivationLabel')}</span>
                </div>
            ...
            <section className="border border-slate-200 dark:border-slate-800 rounded-xl p-8 text-center">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-slate-500 dark:text-slate-500 text-sm">{t('dashboard.visitsInYear')} {currentYear}</p>
              </div>
            </section>
            ...
            <section className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded">{t('dashboard.monSun')}</span>
                </div>
              </div>

              <div className="grid grid-cols-7 gap-2">
                {currentWeekDays.map((date, index) => {
                  const dayName = [t('dashboard.mon'), t('dashboard.tue'), t('dashboard.wed'), t('dashboard.thu'), t('dashboard.fri'), t('dashboard.sat'), t('dashboard.sun')][index];
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400">{t('dashboard.monthlySummary')}</h4>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-lg border border-slate-200 dark:border-slate-800">
                  ...
                  <p className="text-xs font-medium text-slate-700 dark:text-slate-300 mb-0.5">{isCurrentYear ? t('dashboard.thisMonth') : 'Dec'}</p>
                ...
                <div className="p-4 rounded-lg border border-slate-200 dark:border-slate-800">
                  ...
                  <p className="text-xs font-medium text-slate-700 dark:text-slate-300 mb-0.5">{isCurrentYear ? t('dashboard.lastMonth') : 'Nov'}</p>
            ...
            <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
              ...
              <div className="space-y-3">
                <div className="flex items-baseline justify-between">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{t('dashboard.attendancePercentage')}</p>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 text-center pt-1">
                  {totalVisitsYear} {t('dashboard.visitsOf')} {daysElapsed} {t('dashboard.daysElapsed')} {isCurrentYear ? '' : t('dashboard.ofTheYear')}
                </p>
              </div>
            </section>
          </main>
      ...
      {activeTab === 'logs' && (
        <div className="animate-fade-in flex-1 overflow-y-auto px-6 pt-8 pb-24">
          <TabHeader title={t('dashboard.tabAttendance')} onBack={() => setActiveTab('home')} />
        ...
        <div className="animate-fade-in flex-1 overflow-y-auto px-6 pt-8 pb-28 space-y-6">
          ...
          <section className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
            <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <h3 className="font-bold flex items-center gap-2 text-slate-800 dark:text-white">{t('records.bodyMeasurements')}</h3>
              <button onClick={() => setShowMeasurementModal(true)} className="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold">
                {t('records.newMeasurement')}
              </button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-slate-50 dark:bg-slate-800/50">
                  <tr>
                    <th className="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase">{t('records.date')}</th>
                    <th className="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase text-center">{t('records.muscle')}</th>
                    <th className="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase text-center">{t('records.fat')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {processedMeasurements.length === 0 ? (
                    <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-400 text-sm">{t('records.noMeasurements')}</td></tr>
                  ) : (
      ...
      {activeTab === 'settings' && (
        <div className="flex-1 overflow-y-auto px-6 pt-8 pb-28 space-y-6 animate-fade-in">
          <TabHeader title={t('dashboard.tabSettings')} onBack={() => setActiveTab('home')} />
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
                <span className="material-symbols-rounded text-lg text-blue-600">translate</span>
                {t('settings.language')}
              </h3>
            </div>
            <div className="p-3">
                ...
                <button
                  ...
                  className={`py-3 px-4 rounded-lg font-medium text-sm transition-colors ${
                    locale === 'es'
                      ...
                      : 'border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-600'
                  }`}
                >
                  {t('settings.spanish')}
                </button>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2rem] shadow-2xl p-6 relative animate-in slide-in-from-bottom duration-300">
            ...
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{t('records.newMeasurementTitle')}</h3>
            ...
            <div className="space-y-4">
              <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('records.recordDate')}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('records.muscle')}</label>
                ...
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('records.fat')}</label>
                  <div className="flex items-baseline gap-1">
            ...
            <button onClick={handleAddMeasurement} disabled={savingMeasurement} className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl mt-8 shadow-xl shadow-blue-600/20 active:scale-[0.98] transition-all transform disabled:opacity-50">
              {savingMeasurement ? t('records.saving') : t('records.saveMeasurement')}
            </button>
          </div>
        </div>
      )}
