const es = {
  common: {
    loading: 'Cargando...',
    verifying: 'Verificando...',
    save: 'Guardar',
    cancel: 'Cancelar',
    back: '\u2190 Atr\u00e1s',
    continue: 'Continuar \u2192',
    close: 'Cerrar',
    error: 'Error',
    processing: 'Procesando...',
    or: 'o',
    days: 'd\u00edas',
    weeks: 'semanas',
    months: 'meses',
    years: 'a\u00f1os',
    yes: 'S\u00ed',
    no: 'No',
  },
  nav: {
    home: 'Resumen',
    routine: 'Rutina',
    logs: 'Registro',
    kpis: 'KPIs',
    records: 'R\u00e9cords',
    settings: 'Ajustes',
  },
  landing: {
    // Header
    signIn: 'Iniciar Sesión',

    // Hero - punchier, benefit-oriented
    protocolBadge: 'Protocolo de 180 Días',
    heroHeadline: 'Entrená con un plan.',
    heroHeadlineAccent: 'Crecé de verdad.',
    heroSubtitle: 'Protocolos de 180 días que se adaptan a tu objetivo, nivel y disponibilidad. Progresión automática. Tracking completo. Sin adivinar.',
    ctaStart: 'Comenzar 14 días gratis',
    ctaPlans: 'Ver planes',
    trustNoCard: 'Sin tarjeta de crédito',
    trustCancel: 'Cancela cuando quieras',
    trustOffline: 'Funciona offline',

    // Social proof bar
    socialProofUsers: '2,000+',
    socialProofUsersLabel: 'atletas entrenando',
    socialProofWorkouts: '15,000+',
    socialProofWorkoutsLabel: 'sesiones registradas',
    socialProofRating: '4.9',
    socialProofRatingLabel: 'satisfacción promedio',
    socialProofPrograms: '84+',
    socialProofProgramsLabel: 'variantes de plan',

    // Problem / Solution
    problemTitle: '¿Entrás al gym sin un plan?',
    problemSubtitle: 'La mayoría improvisa. Los que progresan siguen un sistema.',
    problemItems: [
      'Sin estructura, repetís los mismos ejercicios sin progresar',
      'Sin tracking, no sabés si realmente estás mejorando',
      'Sin progresión, te estancás en semanas',
    ],
    solutionTitle: 'GymCounter te da el sistema completo',
    solutionItems: [
      'Plan de 180 días adaptado a tu objetivo y nivel',
      'Pesos y series que se ajustan automáticamente cada semana',
      'Registro completo de cada sesión para ver tu evolución real',
    ],

    // How it works
    howItWorksTitle: 'Empezá en 3 minutos',
    howItWorksSubtitle: 'De cero a entrenando con tu plan personalizado',
    step1Title: 'Completá tu perfil',
    step1Desc: 'Contanos tu objetivo, nivel de experiencia y días disponibles por semana.',
    step2Title: 'Recibí tu protocolo',
    step2Desc: '84+ variantes posibles. El tuyo se genera en segundos según tus datos.',
    step3Title: 'Registrá y progresá',
    step3Desc: 'Pesos, series y reps con progresión automática incluida.',

    // Features
    feature1Title: 'Progresión automática',
    feature1Desc: 'Cada semana tus pesos, series y repeticiones se ajustan automáticamente según tu progreso real. Sin calculadoras. Sin adivinar.',
    feature1Bullets: [
      'Pesos calculados por ejercicio',
      'Deload automático cada 4 semanas',
      'Adaptado a tu nivel y objetivo',
    ],
    feature2Title: 'Videos de técnica',
    feature2Desc: 'Cada ejercicio tiene su video de YouTube con la técnica correcta y segura. Ejecutá cada movimiento con confianza.',
    feature2Bullets: [
      'Video integrado por ejercicio',
      'Técnica correcta y segura',
      'Curados por profesionales',
    ],
    feature3Title: 'Funciona sin internet',
    feature3Desc: 'Entrenás en cualquier gym sin preocuparte por la conexión. Todo se sincroniza cuando volvés online.',
    feature3Bullets: [
      'Datos guardados localmente',
      'Sincronización automática',
      'Sin interrupciones en tu entreno',
    ],

    // Goals Section
    goalsTitle: 'Elegí tu camino',
    goalsSubtitle: '8 programas especializados para cada objetivo',
    goalCard_weight_loss: 'Perder Peso',
    goalCard_weight_loss_desc: 'Deficit calórico + cardio estructurado',
    goalCard_muscle_gain: 'Ganar Músculo',
    goalCard_muscle_gain_desc: 'Hipertrofia con volumen optimizado',
    goalCard_max_strength: 'Fuerza Máxima',
    goalCard_max_strength_desc: 'Powerlifting y levantamientos pesados',
    goalCard_conditioning: 'Conditioning/CrossFit',
    goalCard_conditioning_desc: 'Metcons + acondicionamiento funcional',
    goalCard_toned_abs: 'Tonificar Abdomen',
    goalCard_toned_abs_desc: 'Core fuerte + definición abdominal',
    goalCard_glute_building: 'Construir Glúteos',
    goalCard_glute_building_desc: 'Hip thrust + glúteos definidos y fuertes',
    goalCard_fat_burn: 'Quemar Grasa Intenso',
    goalCard_fat_burn_desc: 'HIIT + fuerza para máximo quema calórica',
    goalCard_greek_god: 'Físico Griego',
    goalCard_greek_god_desc: 'Calistenia avanzada + estética: hombros anchos y espalda en V',
    goalCard_postpartum: 'Recuperación posparto',
    goalCard_postpartum_desc: 'Core, suelo pélvico y fuerza progresiva, seguro para diástasis',

    // Stats
    stat1Number: '84+',
    stat1Label: 'variantes de plan',
    stat2Number: '50+',
    stat2Label: 'videos de técnica',
    stat3Number: '180',
    stat3Label: 'días de protocolo',
    stat4Number: '100%',
    stat4Label: 'funciona offline',

    // Testimonials
    testimonialsTitle: 'Lo que dicen nuestros atletas',
    testimonialsSubtitle: 'Resultados reales de personas reales',
    testimonials: [
      {
        name: 'Martín L.',
        role: 'Intermedio · Ganar Músculo',
        quote: 'En 12 semanas subí mi bench de 60kg a 80kg. La progresión automática es un antes y después.',
        metric: '+20kg bench press',
      },
      {
        name: 'Valentina R.',
        role: 'Principiante · Tonificar',
        quote: 'Nunca había seguido un plan más de 2 semanas. Con GymCounter llevo 3 meses sin faltar.',
        metric: '90 días consistente',
      },
      {
        name: 'Diego M.',
        role: 'Avanzado · Fuerza Máxima',
        quote: 'El deload automático y la periodización son exactamente lo que necesitaba. Dejé de estancarme.',
        metric: 'PR en squat y deadlift',
      },
    ],

    // Comparison
    comparisonTitle: 'GymCounter vs. improvisar',
    comparisonWithout: 'Sin sistema',
    comparisonWith: 'Con GymCounter',
    comparisonRows: [
      { without: 'Repetís los mismos pesos', with: 'Progresión automática semanal' },
      { without: 'No sabés si mejoraste', with: 'Tracking completo de cada sesión' },
      { without: 'Te estancás en semanas', with: 'Deload y periodización inteligente' },
      { without: 'Improvisás ejercicios', with: 'Plan estructurado de 180 días' },
      { without: 'Técnica por intuición', with: '50+ videos de técnica profesional' },
    ],

    // Pricing
    pricingTitle: 'Invertí en tu progreso',
    pricingSubtitle: 'Empezá con 14 días gratis. Sin tarjeta de crédito.',
    planMonthly: 'Mensual',
    planAnnual: 'Anual',
    planAnnualSave: 'Ahorrá 2 meses gratis',
    perMonth: '/mes',
    perYear: '/año',
    save: 'Ahorrás',
    mostPopular: 'Más popular',
    billingMonthly: 'Mensual',
    billingAnnual: 'Anual',
    savePercent: 'Ahorrá 17%',
    monthlyFeatures: [
      'Plan personalizado de 180 días',
      'Progresión automática de pesos',
      'Registro completo de sesiones',
      'Videos de técnica por ejercicio',
      'Sistema de logros y badges',
      'Modo offline completo',
    ],
    annualFeatures: [
      'Todo lo del plan mensual',
      'Ahorro de $9.98 al año',
      'Acceso prioritario a nuevas funciones',
      'Soporte prioritario',
      'Estadísticas avanzadas',
      'Sin interrupciones por 12 meses',
    ],
    startFree: 'Comenzar gratis',
    pricingGuarantee: '14 días gratis · Sin tarjeta · Cancelá cuando quieras',
    securePayments: 'Pagos seguros procesados por Stripe · Tus datos están protegidos',

    // FAQ
    faqTitle: 'Preguntas frecuentes',
    faq: [
      { q: '¿Necesito experiencia previa?', a: 'No. GymCounter tiene variantes para principiantes, intermedios y avanzados. El plan se adapta a tu nivel de experiencia.' },
      { q: '¿Qué incluye el período de prueba?', a: 'Acceso completo a todas las funciones durante 14 días. No se requiere tarjeta de crédito para empezar.' },
      { q: '¿Puedo cancelar en cualquier momento?', a: 'Sí. Puedes cancelar tu suscripción cuando quieras desde la configuración. Sin compromisos ni cargos ocultos.' },
      { q: '¿Funciona sin conexión a internet?', a: 'Sí. GymCounter funciona completamente offline. Tus datos se sincronizan automáticamente cuando recuperas la conexión.' },
      { q: '¿Qué pasa cuando termino los 180 días?', a: 'Puedes reiniciar el protocolo con pesos actualizados o mantener tu suscripción para seguir registrando y trackeando tus entrenos.' },
      { q: '¿Los videos de técnica son propios?', a: 'Los videos son de YouTube, curados por profesionales. Cada ejercicio tiene un video de referencia con la técnica correcta.' },
    ],

    // CTA Final
    ctaFinalTitle: '¿Listo para dejar de improvisar?',
    ctaFinalSubtitle: 'Unite a miles de atletas que ya entrenan con un sistema real. 180 días. Tu ritmo. Tu progreso.',
    ctaFinalNoCard: 'Sin tarjeta requerida · Cancelá cuando quieras',

    // Footer
    footerTerms: 'Términos',
    footerPrivacy: 'Privacidad',
    footerContact: 'Contacto',
  },
  auth: {
    signInTitle: 'Iniciar sesi\u00f3n',
    signInSubtitle: 'Accede a tu cuenta para continuar',
    email: 'Email',
    emailPlaceholder: 'tu@email.com',
    password: 'Contrase\u00f1a',
    passwordPlaceholder: '\u00b7\u00b7\u00b7\u00b7\u00b7\u00b7\u00b7\u00b7',
    signInButton: 'Iniciar sesi\u00f3n',
    signingIn: 'Iniciando sesi\u00f3n...',
    continueWithGoogle: 'Continuar con Google',
    noAccount: '\u00bfNo tienes cuenta?',
    registerFree: 'Reg\u00edstrate gratis',
    invalidCredentials: 'Credenciales inv\u00e1lidas',
    errorSignIn: 'Error al iniciar sesi\u00f3n',
    errorGoogle: 'Error iniciando sesi\u00f3n con Google',
    signUpTitle: 'Crear cuenta',
    signUpSubtitle: 'Registrate gratis y empez\u00e1 tu plan',
    fullName: 'Nombre completo',
    fullNamePlaceholder: 'Tu nombre y apellido',
    confirmPassword: 'Confirmar contrase\u00f1a',
    confirmPasswordPlaceholder: '\u00b7\u00b7\u00b7\u00b7\u00b7\u00b7\u00b7\u00b7',
    signUpButton: 'Crear cuenta',
    signingUp: 'Creando cuenta...',
    hasAccount: '\u00bfYa tienes cuenta?',
    signInLink: 'Inicia sesi\u00f3n',
    passwordMismatch: 'Las contrase\u00f1as no coinciden',
    passwordTooShort: 'La contrase\u00f1a debe tener al menos 6 caracteres',
    emailInUse: 'Este email ya est\u00e1 registrado',
    errorSignUp: 'Error al crear cuenta',
  },
  onboarding: {
    stepProfile: 'Perfil',
    stepGoals: 'Objetivos',
    stepPlan: 'Plan',
    footerTip: 'Tu informaci\u00f3n es privada y segura. Puedes cambiarla despu\u00e9s en ajustes.',
    profileTitle: 'Cu\u00e9ntanos sobre ti',
    profileSubtitle: 'Necesitamos algunos datos b\u00e1sicos para personalizar tu plan',
    sex: 'Sexo',
    male: 'Masculino',
    female: 'Femenino',
    weight: 'Peso',
    height: 'Altura',
    age: 'Edad',
    bmi: 'IMC',
    selectSex: 'Por favor selecciona tu sexo',
    goalsTitle: 'Define tu objetivo',
    goalsSubtitle: 'Personalizaremos tu plan seg\u00fan tus metas y disponibilidad',
    mainGoalLabel: '\u00bfCu\u00e1l es tu objetivo principal?',
    goalWeightLoss: 'Perder Peso',
    goalWeightLossDesc: 'D\u00e9ficit cal\u00f3rico + cardio estructurado',
    goalMuscleGain: 'Ganar M\u00fasculo',
    goalMuscleGainDesc: 'Super\u00e1vit cal\u00f3rico + hipertrofia',
    goalMaxStrength: 'Fuerza M\u00e1xima',
    goalMaxStrengthDesc: 'Powerlifting y levantamientos pesados',
    goalConditioning: 'Resistencia/CrossFit',
    goalConditioningDesc: 'Metcons + conditioning funcional',
    goalTonedAbs: 'Tonificar Abdomen',
    goalTonedAbsDesc: 'Core fuerte + definición abdominal',
    goalGluteBuilding: 'Construir Glúteos',
    goalGluteBuildingDesc: 'Hip thrust + glúteos definidos y fuertes',
    goalFatBurn: 'Quemar Grasa Intenso',
    goalFatBurnDesc: 'HIIT + fuerza para máxima quema calórica',
    goalGreekGod: 'Físico Griego',
    goalGreekGodDesc: 'Calistenia avanzada + estética: hombros anchos y espalda en V',
    goalPostpartum: 'Recuperación posparto',
    goalPostpartumDesc: 'Core, suelo pélvico y fuerza progresiva, seguro para diástasis',
    experienceLabel: '\u00bfCu\u00e1l es tu nivel de experiencia?',
    beginner: 'Principiante',
    beginnerDesc: 'Nuevo en el gym o menos de 6 meses',
    intermediate: 'Intermedio',
    intermediateDesc: '6 meses a 2 a\u00f1os de experiencia',
    advanced: 'Avanzado',
    advancedDesc: 'M\u00e1s de 2 a\u00f1os entrenando',
    availabilityLabel: 'D\u00edas disponibles por semana',
    availabilityTip: 'Recomendamos 4-5 d\u00edas para resultados \u00f3ptimos',
    injuriesLabel: '\u00bfTienes alguna lesi\u00f3n o molestia?',
    injuriesOptional: '(opcional)',
    injuriesPlaceholder: 'Ejemplo: Dolor en rodilla derecha, molestia en hombro izquierdo...',
    injuriesTip: 'Adaptaremos los ejercicios para cuidar estas \u00e1reas',
    injuryNone: 'Ninguna',
    injuryKnee: 'Rodilla',
    injuryLowerBack: 'Espalda baja',
    injuryShoulder: 'Hombro',
    injuryWrist: 'Mu\u00f1eca',
    injuryAnkle: 'Tobillo',
    injuryElbow: 'Codo',
    selectGoal: 'Por favor selecciona tu objetivo',
    selectExperience: 'Por favor selecciona tu nivel de experiencia',
    planTitle: 'Tu plan est\u00e1 listo',
    planSubtitle: 'Protocolo de 180 d\u00edas personalizado para ti',
    daysPerWeek: 'd\u00edas/semana',
    totalDays: 'd\u00edas totales',
    firstWeek: 'Tu primera semana',
    dayLabel: 'D\u00eda',
    mainStrength: 'Fuerza Principal',
    accessories: 'Accesorios',
    conditioning: 'Conditioning',
    exercisesTime: '4-6 ejercicios \u00b7 45-60 min',
    trialBadgeTitle: '14 d\u00edas gratis para comenzar',
    trialBadgeDesc: 'Prueba tu plan completo sin compromiso. Despu\u00e9s:',
    trialBadgeNoCard: 'Sin tarjeta requerida',
    startTraining: 'Comenzar entrenamiento',
    configuring: 'Configurando...',
    errorOnboarding: 'Hubo un error al completar el onboarding. Por favor intenta de nuevo.',
    descWeightLoss: 'Plan enfocado en d\u00e9ficit cal\u00f3rico con \u00e9nfasis en cardio y ejercicios compuestos para maximizar gasto cal\u00f3rico',
    descMuscleGain: 'Protocolo de hipertrofia con volumen optimizado y progresi\u00f3n estructurada para ganancia muscular',
    descMaxStrength: 'Programa de fuerza con levantamientos pesados y periodizaci\u00f3n para maximizar tus PRs',
    descConditioning: 'Plan de acondicionamiento funcional con metcons, WODs y ejercicios de alta intensidad',
    descTonedAbs: 'Programa enfocado en fortalecer y definir el abdomen con ejercicios de core, HIIT y circuitos. Ciclos de 6 días con progresión automática.',
    descGluteBuilding: 'Programa especializado en construir y definir glúteos con hip thrust, sentadillas y trabajo de posterior. Ciclos de 6 días con progresión de peso.',
    descFatBurn: 'Programa intensivo de quema de grasa combinando fuerza full body con HIIT, circuitos metabólicos y cardio. Ciclos de 6 días.',
    descGreekGod: 'Plan Full Body de 3 días para construir un físico griego (hombros anchos, espalda en V, pecho superior) con calistenia avanzada. Cada sesión arranca con skills (planche, front lever, muscle-up) y prioriza deltoides laterales. Ciclo de 3 días: Empuje, Tracción y Full Body.',
    descPostpartum: 'Plan de 12 semanas en 3 fases para el posparto: reconecta el core profundo y el suelo pélvico, y recupera fuerza de forma progresiva y segura para la diástasis. En casa con dos mancuernas. Incluye base diaria de reconexión y autotest de diástasis.',
    beginnerFeatures: ['Ejercicios b\u00e1sicos y seguros', 'Volumen moderado para adaptaci\u00f3n', '\u00c9nfasis en t\u00e9cnica correcta', 'Progresi\u00f3n gradual'],
    intermediateFeatures: ['Mayor variedad de ejercicios', 'Volumen y frecuencia \u00f3ptimos', 'T\u00e9cnicas de intensificaci\u00f3n', 'Periodizaci\u00f3n estructurada'],
    advancedFeatures: ['Ejercicios complejos y variaciones', 'Alto volumen de entrenamiento', 'T\u00e9cnicas avanzadas', 'Especializaci\u00f3n y picos'],
  },
  postpartum: {
    phaseLabel: 'Fase {n} de 3',
    phase1Title: 'Fase 1 · Reconexión',
    phase1Tip: 'Semanas 1-3. Foco casi total en la base diaria y la Rutina A. El objetivo es reconectar y llegar a que no aparezca coning en nada.',
    phase2Title: 'Fase 2 · Carga liviana',
    phase2Tip: 'Semanas 4-8. Con buen control, sumás las Rutinas B y C con carga liviana. Subís tiempos, reps y recién ahí un poco de peso.',
    phase3Title: 'Fase 3 · Progresión',
    phase3Tip: 'Semanas 9-12. Progresás a versiones más exigentes y cargas mayores donde el control siga limpio (sin coning, sin pérdidas).',
    goldenRule: 'Regla de oro: exhalá en el esfuerzo y llevá el ombligo hacia adentro. Si aparece un pico o carpa en la panza (coning), volvé a una versión más fácil.',
    baseTitle: 'Base diaria de reconexión',
    baseSubtitle: 'Hacela casi todos los días, incluso los de descanso. Es lo que más mueve la aguja.',
    baseDone: '¡Base de hoy completa! 💪',
    baseReset: 'Se reinicia cada día',
    testTitle: 'Autotest de diástasis',
    testSubtitle: 'Chequeá la separación abdominal antes de arrancar y cada pocas semanas.',
    testIntro: 'Acostada boca arriba, rodillas flexionadas, pies apoyados.',
    testStep1: 'Poné los dedos (horizontales) sobre la línea media, a la altura del ombligo.',
    testStep2: 'Levantá apenas la cabeza y los hombros (inicio de un abdominal).',
    testStep3: 'Sentí cuántos dedos entran en el hueco y si hay profundidad.',
    testStep4: 'Repetí por encima y por debajo del ombligo: la separación no es igual en todos lados.',
    testResultQuestion: '¿Cuántos dedos entran?',
    testMild: '1-2 dedos (leve)',
    testModerate: '2-3+ dedos',
    testUnknown: 'No estoy segura',
    testMildAdvice: 'Separación leve. El plan te sirve tal cual.',
    testModerateAdvice: 'Separación importante. Empezá por la Fase 1 y consultá con un/a kinesiólogo/a de suelo pélvico antes de sumar carga.',
    testUnknownAdvice: 'Lo ideal es una evaluación profesional. El autotest orienta, no diagnostica.',
    testSave: 'Guardar resultado',
    testRetake: 'Volver a testear',
    testLastResult: 'Último resultado',
    testNever: 'Sin registrar',
    testWarningModerate: 'Tu último test marcó una separación importante: sumá carga sólo con control limpio y, si podés, con evaluación profesional.',
    guideTitle: 'Guía posparto',
    rulesTitle: 'Las 5 reglas que no se rompen',
    rule1: 'Exhalar en el esfuerzo y abrazar el ombligo hacia adentro. Nunca aguantar la respiración.',
    rule2: 'Cero coning/doming: si la panza hace un pico, ese ejercicio todavía no toca.',
    rule3: 'Señales de alarma = frenar (pérdidas de orina, pesadez vaginal, dolor lumbar).',
    rule4: 'Costillas sobre pelvis: postura neutra, sin sacar panza ni arquear de más.',
    rule5: 'Progresión, no apuro: dominar la Fase 1 sin coning antes de cargar peso.',
    avoidTitle: 'Ejercicios a evitar (por ahora)',
    avoidBody: 'Abdominales clásicos, sit-ups, planchas completas, flexiones sin modificar, elevar las dos piernas juntas, tijeras y extensiones grandes de yoga (cobra, perro boca arriba). Todo lo que empuje la panza hacia afuera o dispare presión hacia abajo.',
    alarmTitle: 'Señales para frenar',
    alarmBody: 'Pérdidas de orina, sensación de pesadez o presión vaginal, o dolor lumbar: se para y se vuelve a lo básico. Ante la duda, consultá kinesiología de suelo pélvico.',
    walkTitle: 'Caminatas',
    walkBody: 'Caminá 20-40 min la mayoría de los días (se puede con el cochecito). Es lo que realmente empuja el descenso de peso, junto con la alimentación.',
    lactationTitle: 'Lactancia y descenso de peso',
    lactationBody: 'Entrenar no afecta la leche. Bajá gradual (hasta ~0,5 kg/semana), sin dietas restrictivas ni ayunos largos. Priorizá proteína e hidratación. Dar la teta o sacarte leche antes de entrenar, con buen corpiño deportivo.',
    disclaimer: 'Plan orientativo y general. No reemplaza la evaluación de un/a profesional. Ante dolor, pérdidas, cesárea con cicatriz sensible o dudas, consultá kinesiología de suelo pélvico.',
    onboardingTestTitle: 'Antes de arrancar: chequeá tu diástasis',
  },
  dashboard: {
    greeting: '\u00a1Hola, {name}!',
    greetingDefault: 'Atleta',
    motivationLabel: 'Motivaci\u00f3n diaria',
    motivationQuote: '\u201cNo hay l\u00edmites para quien se atreve a superarse.\u201d',
    goodDay: 'Hoy es un buen d\u00eda para entrenar.',
    currentProtocol: 'Protocolo Actual',
    deload: 'Deload',
    day: 'D\u00eda',
    cycle: 'Ciclo',
    visitsInYear: 'Visitas al gym en',
    weekAttendance: 'Asistencia de la semana',
    monthlySummary: 'Resumen mensual',
    thisMonth: 'Este mes',
    lastMonth: 'Mes pasado',
    statistics: 'Estad\u00edsticas',
    attendancePercentage: 'Porcentaje de asistencia',
    visitsOf: 'visitas de',
    daysElapsed: 'd\u00edas transcurridos',
    ofTheYear: 'del a\u00f1o',
    tabAttendance: 'Asistencias',
    tabRoutine: 'Rutina',
    tabProjection: 'Proyecci\u00f3n',
    tabRecords: 'Registros',
    tabSettings: 'Configuraci\u00f3n',
    mon: 'LUN',
    tue: 'MAR',
    wed: 'MI\u00c9',
    thu: 'JUE',
    fri: 'VIE',
    sat: 'S\u00c1B',
    sun: 'DOM',
    monSun: 'Lun - Dom',
  },
  records: {
    militaryProtocol: 'Military Protocol',
    protocolCompleted: 'Protocolo completado',
    trainToday: 'Entrenar hoy',
    viewStructure: 'Ver estructura del protocolo',
    bodyMeasurements: 'Mediciones corporales',
    newMeasurement: '+ Nuevo',
    date: 'Fecha',
    muscle: '% M\u00fasculo',
    fat: '% Grasa',
    noMeasurements: 'Sin mediciones registrados',
    newMeasurementTitle: 'Nueva Medici\u00f3n',
    recordDate: 'Fecha de registro',
    saveMeasurement: 'GUARDAR MEDICI\u00d3N',
    saving: 'Cargando...',
  },
  routine: {
    completeSession: 'COMPLETAR SESI\u00d3N',
    savingSession: 'Guardando...',
    viewTechnique: 'Ver t\u00e9cnica',
    videoNotAvailable: 'Video No Disponible',
    videoNotAvailableDesc: 'Lo sentimos, a\u00fan no tenemos un video para',
    videoNotAvailableDesc2: 'Estamos trabajando en a\u00f1adir m\u00e1s contenido.',
    proTip: 'Pro tip:',
    proTipText: 'Pausa el video y practica la t\u00e9cnica con peso ligero primero',
    howTo: 'C\u00f3mo se hace',
    guideSource: 'Instrucciones: exercises-dataset (MIT)',
    bodyweight: 'Bodyweight',
    startWorkout: 'INICIAR RUTINA',
    inProgress: 'EN CURSO',
    exerciseOf: 'Ejercicio {current} de {total}',
    workoutPreview: 'Ejercicios de hoy',
    upNext: 'Próximos ejercicios',
    skipExercise: 'Saltear ejercicio',
    duration: 'Duración',
    extraWhere: '¿Dónde entrenás hoy?',
    atGym: 'Gimnasio',
    atHome: 'En casa',
    homeEquip: 'Sesión full body en casa: solo tus 2 mancuernas, una soga y tu peso corporal. Sin barra. Elegila las veces que quieras.',
    scheduledSession: 'Sesión del plan',
    quickWorkout: 'Rápido · 20 min',
    quickDesc: 'Sin equipamiento: solo tu cuerpo, ~20 minutos para moverte. Cuenta como sesión completada y avanza el día.',
  },
  settings: {
    title: 'Configuraci\u00f3n',
    logout: 'Cerrar Sesi\u00f3n',
    language: 'Idioma',
    spanish: 'Espa\u00f1ol',
    english: 'English',
    version: 'GymCounter Military v1.0',
    feedbackTitle: 'Feedback y sugerencias',
    feedbackPlaceholder: 'Contanos qué podemos mejorar o qué te gustaría ver...',
    feedbackSend: 'Enviar feedback',
    feedbackSent: 'Enviado, gracias!',
  },
  changePlan: {
    title: 'Plan de Entrenamiento',
    subtitle: 'Cambiá tu objetivo y plan cuando quieras',
    current: 'Plan actual',
    goalLabel: 'Objetivo',
    experienceLabel: 'Nivel',
    daysLabel: 'Días por semana',
    newPlanLabel: 'Nuevo plan',
    changeButton: 'Cambiar de plan',
    confirmTitle: '¿Cambiar tu plan?',
    confirmMessage: 'Tu nuevo plan comenzará desde el Día 1. Tus cargas (pesos base) y todo tu historial se mantienen intactos.',
    confirmCta: 'Sí, cambiar',
    cancel: 'Cancelar',
    success: '¡Plan actualizado! Empezás desde el Día 1.',
    error: 'Error al cambiar el plan. Intentá de nuevo.',
    sameWarning: 'Ya estás en este plan.',
    saving: 'Cambiando...',
  },
  subscription: {
    title: 'Suscripci\u00f3n',
    status: 'Estado',
    plan: 'Plan',
    trial: 'Prueba Gratuita',
    active: 'Activa',
    cancelled: 'Cancelada',
    expired: 'Expirada',
    noSubscription: 'Sin suscripci\u00f3n',
    daysRemaining: 'd\u00edas restantes',
    willNotRenew: 'Tu suscripci\u00f3n no se renovar\u00e1',
    subscribeToAccess: 'Suscr\u00edbete para acceder',
    subscribeToContinue: 'Suscr\u00edbete para continuar',
    monthlyPlan: 'Plan Mensual',
    annualPlan: 'Plan Anual',
    choosePlanTrial: 'Elige un plan para continuar despu\u00e9s de tu prueba:',
    choosePlanExpired: 'Elige un plan para continuar entrenando:',
    annualLabel: 'Anual \u2014 $49.90/a\u00f1o',
    annualBadge: '2 meses gratis',
    monthlyLabel: 'Mensual \u2014 $4.99/mes',
    upgradeToAnnual: 'Cambiar a Anual',
    upgradeBadge: 'Ahorra 2 meses',
    manageBilling: 'Gestionar facturaci\u00f3n',
    errorCheckout: 'Hubo un error. Por favor intenta de nuevo.',
    errorPortal: 'Hubo un error al abrir el portal. Por favor intenta de nuevo.',
  },
  paywall: {
    trialEnded: 'Prueba gratuita finalizada',
    heroTitle: 'Sigue transformando',
    heroTitle2: 'tu f\u00edsico',
    heroSubtitle: 'Contin\u00faa tu progreso con acceso completo desde',
    subscribe: 'Suscribirme',
    securePayment: 'Pago Seguro',
    processedByStripe: 'Procesado por Stripe',
    cancelEasily: 'Cancela F\u00e1cilmente',
    cancelDesc: 'Sin preguntas, desde tu cuenta',
    immediateAccess: 'Acceso Inmediato',
    immediateAccessDesc: 'Contin\u00faa en segundos',
    faqTitle: 'Preguntas frecuentes',
    faq: [
      {
        q: '\u00bfPuedo cancelar en cualquier momento?',
        a: 'S\u00ed, puedes cancelar en cualquier momento desde tu configuraci\u00f3n de cuenta. Mantendr\u00e1s acceso hasta el final de tu per\u00edodo de pago.'
      },
      {
        q: '\u00bfQu\u00e9 m\u00e9todos de pago aceptan?',
        a: 'Aceptamos todas las tarjetas principales (Visa, Mastercard, American Express) procesadas por Stripe.'
      },
      {
        q: '\u00bfPerder\u00e9 mi progreso si no me suscribo?',
        a: 'No. Todo tu progreso se mantiene seguro. Cuando te suscribas, podr\u00e1s continuar donde lo dejaste.'
      }
    ],
  },
  trial: {
    freeTrialPrefix: 'Prueba gratuita:',
    lastDay: '\u00faltimo d\u00eda',
    daysRemaining: 'd\u00edas restantes',
    viewPlans: 'Ver planes',
  },
  guards: {
    verifying: 'Verificando...',
    loading: 'Cargando...',
  },
  freemium: {
    bannerText: 'Tu prueba gratuita termin\u00f3. Suscrib\u00edte para desbloquear todas las funciones.',
    lockedTitle: 'Funci\u00f3n Premium',
    lockedDescription: 'Suscrib\u00edte para acceder a tu rutina de entrenamiento, an\u00e1lisis y r\u00e9cords personales.',
    subscribeCTA: 'Suscribirme para desbloquear',
  },
};

export default es;
